...
# Modèle pour les événements de pilulier
class PillboxEvent(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    status = db.Column(db.String(20), nullable=False)  # Ouvert ou Fermé
...
def simulator():
    ...
    <body>
        ...
        <p>Utilisez ces boutons pour simuler l'ouverture et la fermeture du pilulier:</p>
        
        <button class="button open-button" onclick="sendEvent('opened')">Ouvrir le Pilulier</button>
        <button class="button close-button" onclick="sendEvent('closed')">Fermer le Pilulier</button>
        
        <div class="result" id="result">Résultat apparaîtra ici...</div>
        
        <p><a href="/">Retourner au moniteur</a></p>
            ...
            function sendEvent(status) {
                ...
                .then(data => {
                    document.getElementById('result').textContent = 
                        `Événement envoyé: ${status} - Réponse: ${data.message}`;
                })
                .catch((error) => {
                    document.getElementById('result').textContent = 
                        `Erreur: ${error}`;
...
# API pour recevoir les données de IFTTT via webhook
@app.route('/webhook/pillbox', methods=['POST'])
def pillbox_webhook():
    data = request.json
    
    # Supposons que IFTTT envoie un JSON avec un champ 'value' contenant 'opened' ou 'closed'
    ...
    print(f"Nouvel événement enregistré: Pilulier {status}")
    
    return jsonify({"success": True, "message": f"Pillbox {status} event recorded"})

# API pour obtenir les événements enregistrés
@app.route('/api/events', methods=['GET'])
def get_events():
    # Récupérer tous les événements, ordonner par les plus récents
